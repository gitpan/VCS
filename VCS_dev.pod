...
As noted in the VCS.pm pod one can use C<VCS-E<gt>implementations> to obtain
a list of installed implementations like so:

    #!/usr/local/bin/perl -w
    use strict;
    use VCS;
    my @imps = VCS->implementations();
    for (@imps) {
        print "$_";
    }

One can also prepend new implementations via C<VCS-E<gt>add_implementations>.
...
The modules that make up a VCS implementation act as wrappers
around a command language interpreter (CLI or shell) command.
